...
if __name__ == "__main__":
    ...
    # save net
    torch.save(model, "link_prediction/checkpoints/cora-vgae.pt")
...
# precision:
# 120 epoch, 128 latent size, lr=0.01 - .692~.714 precision, very fast training (.767 on 5th epoch already.)
# 120 epoch, 16 latent size, lr=0.01 - .705~.731 precision, very fast - 5-10 second training.
# 120 epoch, 16 latent size, lr=0.001 - .566~.705- interestingly, training degrades to 50% precision after 100 epoch, from ~.702
# 70 epoch, 16 latent size, lr=0.001 - .710~.712 - ^ stopping early prevents degradation.
# 40 epoch, 16 latent size, lr=0.001 - .702~.709 - Even with 40 epoch. (could probably do only 10 epoch)
# 1 epoch, 16 latent size, lr=0.001 - .709 - 1 epoch.
# 1 epoch, 16 latent size, lr=0.01 - .721 - 1 epoch - bumping up LR didn't help much.
# 1 epoch, 16 latent size, lr=0.1 - .702 - 1 epoch - even with such LR 0.1.
# 1 epoch, 16 latent size, lr=0.3 - .692 - 1 epoch - even with such LR 0.3.
# 1 epoch, 16 latent size, lr=0.8 - .692 - 1 epoch - even with such LR 0.8.
# 1 epoch, 16 latent size, lr=0.00001 - .700 - 1 epoch - even with such LR = 0.00001.
# 200 epoch, 128 latent size, lr=0.01, no beta - can't do the 85% provided in the tutorial?
# 200 epoch, 128 latent size, lr=0.01, regularized KL by num_nodes - .751~.806 - had to regularize KL by node count.
# 200 epoch, 16 latent size, lr=0.01, regularized KL by num_nodes - .733~.769 - less params also works.
